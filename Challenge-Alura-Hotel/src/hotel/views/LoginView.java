...
public class LoginView extends JFrame {
	...
	public LoginView() {
		...
		imgHotel.setIcon(new ImageIcon(LoginView.class.getResource("/hotel/imagenes/img-hotel-login-.png")));
		...
		lblNewLabel_1.setIcon(new ImageIcon(LoginView.class.getResource("/hotel/imagenes/lOGO-50PX.png")));
		...
		JLabel labelRegistro = new JLabel("Si no tienes una cuenta, ingresa tus datos y dale al botón registrar");
		labelRegistro.setFont(new Font("Dialog", Font.PLAIN, 16));
		labelRegistro.setForeground(SystemColor.textInactiveText);
		labelRegistro.setHorizontalAlignment(SwingConstants.CENTER);
		labelRegistro.setBounds(0, 472, 484, 44);
		panel.add(labelRegistro);

		JPanel btnRegistrar = new JPanel();
		btnRegistrar.setBackground(SystemColor.textHighlight);
		btnRegistrar.setLocation(267, 431);
		btnRegistrar.setSize(122, 44);
		...
		btnRegistrar.setBackground(SystemColor.textHighlight);
		btnRegistrar.setBounds(267, 431, 122, 44);
		panel.add(btnRegistrar);
		btnRegistrar.setLayout(null);
		btnRegistrar.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));

		JLabel lblNewLabel_2 = new JLabel("REGISTRAR");
		lblNewLabel_2.setBounds(0, 0, 122, 44);
		btnRegistrar.add(lblNewLabel_2);
	...
	/**
	 * Metodo login que se usa para darle la autorizacion al usuario para ingresar a
	 * la aplicacion del Hotel Alura, hace uso de los metodos {@code obtenerClave()}
	 * que sirve para comparar la contraseña ingresada en la aplicacion con la que
	 * esta en la base de datos, todo estos con el encriptador de BCrypt
	 */
	@SuppressWarnings("deprecation")
	private void Login() {
		String usuario = txtUsuario.getText().toString();
		String userInputPassword = txtContrasena.getText().toString();

		try {
			// Obtener la contraseña almacenada en la base de datos para el usuario
			String storedHashedPassword = this.loginController.obtenerClave(usuario);
	...
	/**
	 * Metodo guardar que sirve para guardar un nuevo usuario al no tener cuenta para hacer valido el login en el Hotel Alura
	 */
	@SuppressWarnings("deprecation")
	private void guardar() {
		...
		this.loginController.guardar(user, clave);
	}
